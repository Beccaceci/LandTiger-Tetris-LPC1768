...
void PutChar (uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor) {
    ...
    GetASCIICode(buffer, ASCI);  // get the font bitmap for this character
    
    for(i = 0; i < 16; i++) {
        LCD_SetCursor(Xpos, Ypos + i); // set cursor once at the start of the current row
        LCD_WriteIndex(0x0022); // prepare to write to GRAM
        
        tmp_char = buffer[i];
        
        // stream the 8 pixels of this row directly
        for(j = 0; j < 8; j++) {
            // check if the bit is 1 (Foreground) or 0 (Background)
            if(((tmp_char >> (7 - j)) & 0x01) == 0x01)
                LCD_WriteData(charColor); // fast write without setting cursor
            else
                LCD_WriteData(bkColor);   // fast write without setting cursor
        }
    }
}
...
* Description    : Optimized rectangle drawing. Sets cursor once per row.
* Input          : - Xpos: Start X
* - Ypos: Start Y
...
void LCD_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t width, uint16_t height, uint16_t color) {
    ...
    for(i = 0; i < height; i++) {
        LCD_SetCursor(Xpos, Ypos + i); // set cursor to the start of the current row
        LCD_WriteIndex(0x0022); // prepare to write to GRAM (Register 0x22)
        
        // stream the color data for the whole row
        for(j = 0; j < width; j++)
            LCD_WriteData(color);
    }
}
